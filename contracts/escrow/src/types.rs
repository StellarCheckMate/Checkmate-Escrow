...
pub enum DataKey {
    ...
    SnapshotCount(u64),
    /// Player-level balance snapshot at a ring-buffer slot.
    /// The `u64` is the slot computed as `(snapshot index) % MAX_PLAYER_SNAPSHOTS`.
    /// Used by `get_balance_at_timestamp` for point-in-time balance queries.
    /// See lib.rs `record_player_snapshot` for the storage scheme.
    PlayerBalanceSnapshot(Address, u64),
    /// Total number of player-balance snapshots ever recorded for a player
    /// (monotonic, never reset). Bumps on every deposit / payout / refund.
