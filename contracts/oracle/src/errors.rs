...
pub enum Error {
    ...
    MatchNotDisputed = 16,
    /// A swap was attempted with an invalid (non-positive) amount.
    InvalidAmount = 17,
    /// An arithmetic overflow occurred during a swap calculation.
    Overflow = 18,
    /// The computed output amount is below the caller's `min_amount_out` threshold.
    SlippageExceeded = 19,
}
