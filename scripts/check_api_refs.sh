...
# Build allowlist from actual public contract functions and shared test helpers
ALLOWLIST=$(grep -rh "pub fn " \
  "$REPO_ROOT/contracts/escrow/src/lib.rs" \
  "$REPO_ROOT/contracts/oracle/src/lib.rs" \
  "$REPO_ROOT/contracts/escrow/src/tests/helpers.rs" \
...
while IFS= read -r file; do
  while IFS= read -r name; do
    base="${name#try_}"
    if ! echo "$ALLOWLIST" | grep -qx "$name" && ! echo "$ALLOWLIST" | grep -qx "$base"; then
