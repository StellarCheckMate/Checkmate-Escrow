...
| `get_completed_matches` | `() -> Result<Vec<Match>, Error>` | Returns all matches in `Completed` state. **Hard scan cap:** if the total `MatchCount` exceeds 500, returns `Error::TooManyActiveMatches` immediately (O(1) check) and emits a `"scan" / "cap_hit"` diagnostic event carrying the current match count. Below the cap it still scans in linear time. Use `match_count_exceeds_scan_cap()` as a cheap pre-flight check; switch to `get_completed_matches_paginated` before the cap is reached. |
| `match_count_exceeds_scan_cap` | `() -> bool` | Returns `true` if the total `MatchCount` already exceeds the scan cap (500), meaning a call to `get_completed_matches` will fail. O(1) instance-storage read — use as a pre-flight check before calling `get_completed_matches`. |
| `get_completed_matches_paginated` | `(offset: u32, limit: u32) -> Result<Vec<Match>, Error>` | Paginated version of `get_completed_matches`, ordered by match ID ascending. **Production-safe path** — unaffected by the scan cap. Prefer this function over `get_completed_matches` for any deployment where the match count could grow over time. |
