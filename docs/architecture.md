...
| `submit_result_consensus` | `(match_id: u64, winner: Winner, oracle_address: Address) -> Result<(), Error>` | Any approved oracle votes on a match outcome; each oracle may vote once per match and all votes must agree (a conflicting vote returns `Error::ConflictingResult`). Once the required confirmation threshold is reached, payout executes automatically. A conflicting vote is still recorded (queryable via `get_rejected_oracle_vote`) and, if the confirmation threshold becomes mathematically unreachable given the remaining unvoted approved oracles, the match is flagged deadlocked (see `is_oracle_deadlocked`) and an `ora_dead` event is emitted. |
| `get_oracle_confirmations` | `(match_id: u64) -> u32` | Returns the current confirmation count for a match. |
| `get_rejected_oracle_vote` | `(match_id: u64, oracle_address: Address) -> Option<Winner>` | Returns the winner an oracle voted for if that vote conflicted with the leading vote and was rejected (not counted toward consensus), or `None` if that oracle has no rejected vote on record for this match. |
| `is_oracle_deadlocked` | `(match_id: u64) -> bool` | Returns whether `submit_result_consensus` has flagged this match as deadlocked — the required confirmation threshold can no longer be reached given conflicting votes already cast and the number of approved oracles yet to vote. |
