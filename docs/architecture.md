...
| `get_completed_matches` | `() -> Result<Vec<Match>, Error>` | Returns all matches in `Completed` state. **Enforces a hard scan cap of `GET_COMPLETED_MATCHES_SCAN_CAP` (500) total matches**: once the contract's match count reaches this threshold, the function returns `Error::TooManyResults` (#45) on every call and emits a `query / scan_cap_hit` diagnostic event carrying the current match count as payload. This replaces silent resource-exhaustion degradation with an actionable on-chain signal. **Switch to `get_completed_matches_paginated` before hitting this threshold** — it is the production-safe path, imposes no scan cap, and is unambiguously documented as the canonical approach for any deployment expected to grow over time. |
| `get_completed_matches_paginated` | `(offset: u32, limit: u32) -> Result<Vec<Match>, Error>` | **Production-safe path.** Paginated version of `get_completed_matches`, ordered by match ID ascending. Unaffected by the scan cap: it performs a bounded scan using `collect_matches_by_state_paginated` and never emits `query/scan_cap_hit`. Use this for all production callers — paginate with `offset`/`limit` to retrieve completed match history at any scale. |
