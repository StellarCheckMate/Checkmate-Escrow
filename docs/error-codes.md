...
| 45 | `TooManyActiveMatches` | Match creation; `get_completed_matches` (scan cap) | **Two causes:** (1) The player has exceeded the maximum concurrent active matches (`MAX_ACTIVE_MATCHES_PER_PLAYER`). (2) `get_completed_matches` was called when the total `MatchCount` exceeds the scan cap (500) — the call aborts before scanning to avoid resource exhaustion and emits a `"scan" / "cap_hit"` diagnostic event. | Cause 1: wait for some matches to complete or cancel. Cause 2: switch to `get_completed_matches_paginated`; use `match_count_exceeds_scan_cap()` as a cheap pre-flight check to avoid hitting the cap at all. | Cause 1 — player with 1 000 active matches tries to create a 1 001st → `#45`. Cause 2 — `get_completed_matches` called when total match count is 501 → `#45` + `"scan"/"cap_hit"` event. |
