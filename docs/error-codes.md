...
| 15 | `InvalidGameId` | [`create_match`](../contracts/escrow/src/lib.rs) | `game_id` is empty, longer than 64 bytes, or doesn't match the format expected for `platform`: **Lichess** requires exactly 8 ASCII alphanumeric characters; **Chess.com** requires 7–12 ASCII digits. | Pass a `game_id` in the format expected for the chosen `platform` (see above), under the 64-byte limit. | `create_match` with `game_id = ""` → `#15`. `create_match` with `platform = Platform::Lichess` and `game_id = "12a4567"` (7 chars) → `#15`. |
